...
const newDriverHistory = [
  driverLap('New Driver', 1, [30000, 40000, 30000]),
  driverLap('New Driver', 2, [31000, 41000, 31000]),
  driverLap('New Driver', 3, [32000, 42000, 32000]),
  driverLap('New Driver', 4, [33000, 43000, 33000]),
  driverLap('New Driver', 5, [34000, 44000, 34000])
];
prediction = buildLapPrediction({
...
prediction = buildLapPrediction({
  history: [driverLap('Old Driver', 1, [30000, 40000, 30000])],
...
prediction = buildLapPrediction({
  history: [
    driverLap('Returning Driver', 1, [29000, 39000, 29000]),
    driverLap('Other Driver', 2, [35000, 45000, 35000]),
    driverLap('Returning Driver', 3, [31000, 41000, 31000])
  ],
...
const dryHistory = Array.from({ length: 10 }, (_, index) => driverLap('Weather Driver', index + 1, [30000, 40000, 30000]));
prediction = buildLapPrediction({
  history: dryHistory,
...
const mixedWeatherHistory = [
  ...dryHistory,
  ...Array.from({ length: 5 }, (_, index) => driverLap('Weather Driver', 11 + index, [35000, 50000, 40000]))
];
prediction = buildLapPrediction({
...
const wetHistory = [
  ...dryHistory,
  ...Array.from({ length: 10 }, (_, index) => driverLap('Weather Driver', 11 + index, [35000, 50000, 40000]))
];
prediction = buildLapPrediction({
  history: wetHistory,
  rows: [liveRow({ driver: 'Weather Driver', sector1: '35.000' })],
...
const neutralizedHistory = [
  driverLap('Flag Driver', 1, [30000, 40000, 30000]),
  driverLap('Flag Driver', 2, [28000, 80000, 70000], {
...
prediction = buildLapPrediction({
  history: [driverLap('Flag Driver', 1, [30000, 40000, 30000])],
