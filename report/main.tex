% !TeX root = main.tex
% !TeX program = latexmk
\documentclass[11pt,a4paper]{report}

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[dutch]{babel}
\usepackage{lmodern}
\usepackage{geometry}
\usepackage{setspace}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{array}
\usepackage{tabularx}
\usepackage{enumitem}
\usepackage{xcolor}
\usepackage{hyperref}
\usepackage{pdfpages}
\usepackage{titlesec}

\geometry{left=2.7cm,right=2.7cm,top=2.5cm,bottom=2.5cm}
\onehalfspacing
\setlength{\parskip}{0.6em}
\setlength{\parindent}{0pt}
\emergencystretch=2em
\graphicspath{{figures/}}

\definecolor{pendingred}{RGB}{170,35,45}
\newcommand{\pending}[1]{%
  \par\noindent\fcolorbox{pendingred}{pendingred!8}{%
    \parbox{0.94\textwidth}{\textbf{NOG UIT TE VOEREN:} #1}}\par}
% detokenize makes file names such as lap_history.jsonl safe in normal text.
\newcommand{\code}[1]{\texttt{\detokenize{#1}}}

\hypersetup{
  colorlinks=true,
  linkcolor=black,
  citecolor=black,
  urlcolor=blue,
  pdftitle={Stageverslag Industriële Stage - Race Engineer Dashboard},
  pdfauthor={Jeff Mathieu}
}

\titleformat{\chapter}[display]
  {\normalfont\huge\bfseries}
  {\chaptertitlename\ \thechapter}{20pt}{\Huge}

\begin{document}

\pagenumbering{gobble}
\input{chapters/00-frontmatter}
\tableofcontents
\clearpage
\pagenumbering{arabic}

\input{chapters/01-inleiding-en-context}
\input{chapters/02-requirements-en-proces}
\input{chapters/03-architectuur}
\input{chapters/03b-alternatieven}
\input{chapters/04-data-en-opslag}
\input{chapters/05-analyse-en-strategie}
\input{chapters/06-interface}
\input{chapters/07-testen-en-oplevering}
\input{chapters/08-evaluatie-en-reflectie}
\input{chapters/08b-relatie-met-opleiding}
\input{chapters/09-spa-validatie}
\input{chapters/10-conclusie}
\input{chapters/11-referenties}

\appendix
\chapter{Stageplan}
\includepdf[pages=-,pagecommand={}]{appendices/stageplan_Jeff_Mathieu.pdf}

\chapter{Competenties die ik verwachtte te verbeteren}
\includepdf[pages=-,pagecommand={}]{appendices/competenties_Jeff_Mathieu.pdf}

\end{document}
